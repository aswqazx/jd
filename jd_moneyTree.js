...
const JD_API_HOST2 = 'https://ms.jr.jd.com/gw/generic/mission/h5/m';
  ...
  return new Promise(async resolve => {
    ...
    if (response && response.resultCode === 0) {
      if (response.resultData.code === '200') {
        response.resultData.data.map((item) => {
          ...
          if (item.workType === 6 && item.prizeType === 0) {
    ...
    for (let task of taskInfo) {
      if (task.mid && task.workStatus === -1) {
        ...
        let aa = await queryMissionReceiveAfterStatus(task.mid);
        console.log(`${JSON.stringify(aa)}`);
        await $.wait(1000 * 16);
        let bb = await finishReadMission(task.mid);
...
// 浏览任务
async function finishReadMission(missionId) {
  const params = {
    "missionId": missionId,
    "readTime": 15
  }
  let response = await request2('finishReadMission', JSON.stringify(params))
  console.log(`missionId为${missionId}：：浏览活动完成: ${JSON.stringify(response)}`);
}
async function queryMissionReceiveAfterStatus(missionId) {
  const params = {
    "missionId": missionId
  }
  console.log(JSON.stringify(params))
  let response = await request2('queryMissionReceiveAfterStatus', JSON.stringify(params))
  console.log(`missionId为${missionId}：：queryMissionReceiveAfterStatus: ${JSON.stringify(response)}`)
}
...
async function request2(missionId, reqData) {
  await $.wait(1000); //歇口气儿, 不然会报操作频繁
  return new Promise((resolve, reject) => {
    $.get(taskurl2(missionId, reqData), (err, resp, data) => {
      try {
        if (err) {
          console.log("\n摇钱树京东API请求失败 ‼️‼️");
          console.log(JSON.stringify(err));
...
function taskurl2(function_id, reqData) {
  return {
    url: JD_API_HOST2 + '/' + function_id + '?reqData=' + encodeURIComponent(reqData),
    headers: {
      'Accept': `*/*`,
