...
async function doTask(){
  for (let i = 0; i < $.taskList.length; i++) {
    ...
    if($.oneTask.assignmentType === 0){
      console.log(`任务：${$.oneTask.assignmentName}，去执行`);
      await takePostRequest('share');
    }
...
async function takePostRequest(type) {
  ...
  switch (type) {
    ...
    case 'help':
      ...
      break;
    case 'share':
      url = `https://api.m.jd.com/api?functionId=superBrandDoTask&appid=ProductZ4Brand&client=wh5&t=${Date.now()}&body=%7B%22source%22:%22secondfloor%22,%22activityId%22:${$.activityId},%22encryptProjectId%22:%22${$.encryptProjectId}%22,%22encryptAssignmentId%22:%22${$.oneTask.encryptAssignmentId}%22,%22assignmentType%22:0,%22completionFlag%22:1,%22itemId%22:null,%22actionType%22:0%7D`;
      break;
    default:
      console.log(`错误${type}`);
  }
